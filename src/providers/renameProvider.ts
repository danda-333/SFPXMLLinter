...
import { getIndexedForms } from "../core/model/indexAccess";
...
export class SfpXmlRenameProvider implements vscode.RenameProvider {
  ...
  public async prepareRename(
    ...
    const target = this.withConsistentSnapshot(() => this.resolveRenameTarget(document, position));
    if (!target) {
  ...
  public async provideRenameEdits(
    ...
    if (target.kind === "form") {
      for (const location of collectFormIdentReferenceLocations(index, target.oldIdent, this.getFactsForUri)) {
    ...
    for (const location of collectWorkflowReferenceLocations(index, target.formIdent, formKind, target.oldIdent, this.getFactsForUri)) {
      pushRenameLocation(edit, seen, location, normalizedNewName);
    }
    if (target.kind === "control") {
      for (const location of collectHtmlControlReferenceLocations(index, target.formIdent, target.oldIdent, this.getFactsForUri)) {
        pushRenameLocation(edit, seen, location, normalizedNewName);
      }
    }
  ...
  private resolveRenameTarget(document: vscode.TextDocument, position: vscode.Position): RenameTarget | undefined {
    ...
    if (formEntry) {
      const declaration = findFormDeclaration(index, formEntry.ident, this.getFactsForUri) ?? formEntry.formIdentLocation;
